...
# PARITY CRITICAL: Import from shared core module to ensure identical behavior
# These imports will shadow any local definitions below, ensuring parity with FastAPI
from core_shared.edi_core import (
    PROMPTS, fill_prompt, remove_legacy_banner,
    generate_xslt_smart, merge_xslt_contract,
    edi_to_xml, transform_with_xslt,
    detect_standard, parse_structured_segments,
    X12_STANDARD_TAGS, EDIFACT_STANDARD_TAGS,
    CROSS_FORMAT_TAG_MAP, CROSS_FORMAT_QUAL_MAP,
    LLMEngine, StructuralMatcher, RAGDatabase, Config,
    query_coverage_score_hybrid, extract_metadata_deterministic,
    extract_customer_info, build_signal_text,
...
def transform_with_xslt(xml_content: str, xslt_content: str, use_saxonc: bool = False) -> Dict[str, str]:
        ...
        try:
            ...
            with PySaxonProcessor(license=False) as proc:
                ...
                try:
                    ...
                    return {"success": False, "output": f"XML Validation Error: {str(xml_err)}", "method": "SaxonC"}
                ...
                try:
                    ...
                    return {"success": False, "output": f"XSLT Output XML is malformed: {str(xml_err)}", "method": "SaxonC"}
    ...
    # Fallback to lxml when SaxonC is not available or use_saxonc=False
    try:
        from lxml import etree
        
        # Parse the XSLT stylesheet
        xslt_root = etree.XML(xslt_content.encode('utf-8'))
        transform = etree.XSLT(xslt_root)
        
        # Parse the XML document
        xml_root = etree.XML(xml_content.encode('utf-8'))
        
        # Perform the transformation
        result_tree = transform(xml_root)
        result_string = str(result_tree)
        
        return {"success": True, "output": result_string, "method": "lxml"}
        
    except Exception as e:
        return {"success": False, "output": f"lxml Error: {str(e)}", "method": "lxml"}
...
def init_session_state():
    ...
    st.session_state.setdefault("conv", {
        "session_id": str(uuid.uuid4()),
        "customer": {"type": None, "name": None},
        "upload": None,  # {filename, sha256, size, text, meta}
        "matches": None,  # [cards...]
        "chosen_match": None,  # a card
        "analysis": None,  # coverage dict
        "xslt": {
            "template": None,  # {id, path, changes, compliance}
            "final": None  # {id, path, changes, diff_summary}
        },
        "idoc": None  # {id, path}
    })
    ...
    st.session_state.setdefault("cache", {
        ...
        "match_inputs": None,
        "xslt_inputs": None,
        "automerge_inputs": None,
    })

    st.session_state.setdefault("chat_messages", [])  # For chat display

# =========================================================================================
# CONVERSATIONAL INTERFACE FUNCTIONS
# =========================================================================================

def get_next_step(conv) -> dict:
    """Determine the current step in the workflow and return prompt/options"""
    if not conv["upload"]:
        return {
            "step": "intake",
            "prompt": "👋 Welcome! Let's start by uploading your EDI file to find the best matching templates.",
            "options": ["Upload EDI File"]
        }
    
    if not conv["matches"]:
        return {
            "step": "match",
            "prompt": f"📄 Great! I've processed your file '{conv['upload']['filename']}'. Ready to search for matching templates?",
            "options": ["🔍 Find Matches"]
        }
    
    if not conv["chosen_match"]:
        return {
            "step": "choose_match",
            "prompt": f"🎯 Found {len(conv['matches'])} potential matches! Please select the best template to proceed.",
            "options": ["Select Match"]
        }
    
    if not conv["analysis"]:
        return {
            "step": "analyze",
            "prompt": f"✅ Selected template for {conv['chosen_match']['customer_info'].get('customer_name', 'Unknown')}. Let's analyze the coverage.",
            "options": ["🔬 Analyze Coverage"]
        }
    
    if not conv["xslt"]["template"]:
        return {
            "step": "xslt_generate",
            "prompt": f"📊 Coverage analysis complete! Ready to generate a customized XSLT template?",
            "options": ["✨ Generate XSLT"]
        }
    
    if not conv["xslt"]["final"]:
        return {
            "step": "automerge",
            "prompt": f"🎨 XSLT template generated! Let's merge it with the original template.",
            "options": ["🔄 Auto-Merge"]
        }
    ...
    if not conv["idoc"]:
        return {
            "step": "idoc",
            "prompt": f"🎉 Merge complete! Ready to transform your EDI and generate the final IDoc?",
            "options": ["🚀 Build IDoc"]
        }
    
    return {
        "step": "done",
        "prompt": f"🎊 Workflow complete! Your IDoc is ready for download.",
        "options": ["📥 Download IDoc", "🔄 Start Over"]
    }

def format_message(role, content, message_type=None):
    ...
    return {
        ...
        "type": message_type  # "text", "matches", "analysis", "download"
    }

def add_chat_message(role, content, message_type=None):
    """Add a message to the chat history"""
    message = format_message(role, content, message_type)
    st.session_state.chat_messages.append(message)

def handle_upload_edi(file):
    """Handle EDI file upload and extract metadata"""
    import hashlib
    ...
    try:
        ...
        meta = {
            ...
            "format": "EDI" if any(x in file_content for x in ['ISA', 'ST', 'GS']) else "Unknown"
        }
        
        # Update conversation state
        st.session_state.conv["upload"] = {
            "filename": file.name,
            "sha256": file_hash,
            "size": meta["size"],
            "text": file_content,
        ...
        st.session_state.conv["matches"] = None
        st.session_state.conv["chosen_match"] = None
        st.session_state.conv["analysis"] = None
        st.session_state.conv["xslt"] = {"template": None, "final": None}
        st.session_state.conv["idoc"] = None
        
        # Clear cache
        st.session_state.cache["upload_hash"] = file_hash
        st.session_state.cache["match_inputs"] = None
        st.session_state.cache["xslt_inputs"] = None
        st.session_state.cache["automerge_inputs"] = None
        
        # Add success message
        add_chat_message("assistant", f"✅ Successfully uploaded '{file.name}' ({meta['size']:,} characters, {meta['lines']} lines)", "text")
        
        return True
        
    except Exception as e:
        st.error(f"❌ Upload failed: {str(e)}")
        add_chat_message("assistant", f"❌ Upload failed: {str(e)}", "text")
        return False

def handle_show_matches():
    """Handle match search with caching"""
    import hashlib
    
    try:
        upload_text = st.session_state.conv["upload"]["text"]
        top_k = 3  # Default to top 3 matches
        
        # Check cache
        input_hash = hashlib.sha256(f"{upload_text}top{top_k}".encode()).hexdigest()
        if st.session_state.cache["match_inputs"] == input_hash and st.session_state.conv["matches"]:
            matches = st.session_state.conv["matches"]
            add_chat_message("assistant", f"📋 Using cached results - found {len(matches)} matches", "text")
        else:
            # Run search
            with st.spinner("🔍 Searching for matches..."):
                matches = st.session_state.rag_db.search(upload_text, top_k)
            
            # Update state and cache
            st.session_state.conv["matches"] = matches
            st.session_state.cache["match_inputs"] = input_hash
            
            add_chat_message("assistant", f"🎯 Found {len(matches)} potential matches!", "matches")
        
        return True
        
    except Exception as e:
        st.error(f"❌ Search failed: {str(e)}")
        add_chat_message("assistant", f"❌ Search failed: {str(e)}", "text")
        return False

def handle_choose_match(match_id):
    """Handle match selection"""
    try:
        matches = st.session_state.conv["matches"]
        if 0 <= match_id < len(matches):
            chosen_match = matches[match_id]
            st.session_state.conv["chosen_match"] = chosen_match
            
            customer_name = chosen_match['customer_info'].get('customer_name', 'Unknown')
            score = chosen_match.get('structural_score', 0.0)
            
            add_chat_message("assistant", f"✅ Selected template for {customer_name} (Score: {score:.1f}%)", "text")
            return True
        else:
        ...
        add_chat_message("assistant", f"❌ Selection failed: {str(e)}", "text")
        return False
...
def handle_analyze_coverage():
    ...
    try:
        upload_text = st.session_state.conv["upload"]["text"]
        chosen_match = st.session_state.conv["chosen_match"]
        customer_name = chosen_match['customer_info'].get('customer_name', 'Unknown')
        
        with st.spinner("🔬 Analyzing coverage..."):
            analysis = query_coverage_score_hybrid(
                upload_text,
                chosen_match['document']['content'],
                st.session_state.rag_db.matcher  # Use the matcher from RAG database
            )
        
        st.session_state.conv["analysis"] = analysis
        add_chat_message("assistant", f"📊 Coverage analysis complete for {customer_name}", "analysis")
        return True
        
    except Exception as e:
        st.error(f"❌ Analysis failed: {str(e)}")
        add_chat_message("assistant", f"❌ Analysis failed: {str(e)}", "text")
        return False

def handle_generate_xslt():
    """Handle XSLT generation"""
    import hashlib
    ...
    try:
        ...
        xslt_content = chosen_match['document'].get('xslt_content')
        if not xslt_content:
            st.error("❌ No base XSLT content found")
            add_chat_message("assistant", "❌ No base XSLT content found", "text")
            return False
        
        # Check cache for XSLT generation
        input_hash = hashlib.sha256(f"{xslt_content}{upload_text}".encode()).hexdigest()
        if st.session_state.cache["xslt_inputs"] == input_hash and st.session_state.conv["xslt"]["template"]:
            add_chat_message("assistant", "📋 Using cached XSLT generation results", "text")
            return True
        ...
        if generation_result.get('error'):
            ...
            add_chat_message("assistant", f"❌ LLM API Error: {generation_result['error']}", "text")
            return False
        
        # Store template with actual content
        st.session_state.conv["xslt"]["template"] = {
            "id": f"template_{st.session_state.conv['session_id']}",
            "path": None,  # Will be set when saved
            "changes": generation_result.get('changes', []),
            "compliance": generation_result.get('compliance', {}),
            "content": generation_result.get('modified_xslt', xslt_content)  # Store actual XSLT content
        }
        
        # Update cache
        st.session_state.cache["xslt_inputs"] = input_hash
        
        add_chat_message("assistant", f"✨ XSLT template generated with {len(generation_result.get('changes', []))} changes", "xslt_template")
        return True
        
    except Exception as e:
        st.error(f"❌ XSLT generation failed: {str(e)}")
        add_chat_message("assistant", f"❌ XSLT generation failed: {str(e)}", "text")
        return False

def handle_automerge():
    """Handle XSLT auto-merge using existing proven merge logic"""
    import hashlib
    from datetime import datetime
    
    try:
        # Check cache for auto-merge
        template_id = st.session_state.conv["xslt"]["template"]["id"]
        input_hash = hashlib.sha256(f"automerge_{template_id}".encode()).hexdigest()
        
        if st.session_state.cache["automerge_inputs"] == input_hash and st.session_state.conv["xslt"]["final"]:
            add_chat_message("assistant", "📋 Using cached auto-merge results", "text")
            return True
        
        # Get the generated XSLT template
        template = st.session_state.conv["xslt"]["template"]
        generated_xslt = template.get('content', '')
        
        if not generated_xslt:
            st.error("❌ No generated XSLT template found")
            add_chat_message("assistant", "❌ No generated XSLT template found", "text")
            return False
        
        # Get the original XSLT from the chosen match
        chosen_match = st.session_state.conv["chosen_match"]
        original_xslt = chosen_match['document'].get('xslt_content', '')
        
        if not original_xslt:
            st.error("❌ No original XSLT template found")
            add_chat_message("assistant", "❌ No original XSLT template found", "text")
            return False
        
        # Get coverage analysis to understand missing elements
        analysis = st.session_state.conv.get("analysis", {})
        missing_tags = analysis.get('missing_tags', [])
        missing_tqs = analysis.get('missing_tqs', [])
        
        # Format missing elements for the existing merge prompt
        missing_elements = []
        for tag in missing_tags:
            missing_elements.append(tag)
        for tq in missing_tqs:
            if len(tq) >= 2:
                missing_elements.append(f"{tq[0]}+{tq[1]}")
        
        # Use the existing proven merge logic
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        
        merge_prompt = fill_prompt(
            PROMPTS["merge_xslts"],
            primary_xslt=original_xslt,
            supplementary_xslt=generated_xslt,
            missing_elements=missing_elements,
            timestamp=timestamp
        )
        
        with st.spinner("🔄 Merging XSLT templates..."):
            merge_result = st.session_state.llm_engine.call_llm(merge_prompt, use_indexing_model=True)
        
        if merge_result.get('error'):
            st.error(f"❌ Merge failed: {merge_result['error']}")
            add_chat_message("assistant", f"❌ Merge failed: {merge_result['error']}", "text")
            return False
        
        # Remove legacy banner comments from merged XSLT (existing logic)
        if "merged_xslt" in merge_result:
            merge_result["merged_xslt"] = remove_legacy_banner(merge_result["merged_xslt"])
        
        # Validate the merged XSLT before storing
        merged_xslt_content = merge_result.get('merged_xslt', '')
        if merged_xslt_content:
            try:
                from lxml import etree
                # Try to parse the XSLT to catch syntax errors early
                etree.XML(merged_xslt_content.encode('utf-8'))
                add_chat_message("assistant", "✅ Merged XSLT syntax validation passed", "text")
            except Exception as validation_error:
                st.warning(f"⚠️ Merged XSLT has syntax issues: {str(validation_error)}")
                add_chat_message("assistant", f"⚠️ Merged XSLT has syntax issues: {str(validation_error)}", "text")
                # Still store it but with a warning
        
        # Store the merged result
        st.session_state.conv["xslt"]["final"] = {
            "id": f"final_{st.session_state.conv['session_id']}",
            "path": None,
            "changes": [],
            "diff_summary": merge_result.get('explanation', 'Auto-merge completed'),
            "content": merged_xslt_content  # Store actual merged content
        }
        
        # Update cache
        st.session_state.cache["automerge_inputs"] = input_hash
        
        add_chat_message("assistant", "🔄 Auto-merge completed successfully", "xslt_final")
        return True
        
    except Exception as e:
        st.error(f"❌ Auto-merge failed: {str(e)}")
        add_chat_message("assistant", f"❌ Auto-merge failed: {str(e)}", "text")
        return False

def handle_build_idoc():
    """Handle IDoc generation"""
    try:
        # Get the merged XSLT template
        final_xslt = st.session_state.conv["xslt"]["final"]
        merged_xslt_content = final_xslt.get('content', '')
        
        if not merged_xslt_content:
            st.error("❌ No merged XSLT template found")
            add_chat_message("assistant", "❌ No merged XSLT template found", "text")
            return False
        
        # Get the original EDI content
        upload_text = st.session_state.conv["upload"]["text"]
        
        if not upload_text:
            st.error("❌ No EDI content found")
            add_chat_message("assistant", "❌ No EDI content found", "text")
            return False
        
        with st.spinner("🚀 Transforming EDI to IDoc..."):
            # Convert EDI to proper XML format using the original edi_to_xml function
            xml_content = edi_to_xml(upload_text)
            
            # Check if conversion was successful
            if xml_content.startswith("<e>"):
                st.error(f"❌ EDI to XML conversion failed: {xml_content}")
                add_chat_message("assistant", f"❌ EDI to XML conversion failed: {xml_content}", "text")
                return False
            
            # Perform the transformation using the original logic
            transform_result = transform_with_xslt(xml_content, merged_xslt_content, use_saxonc=False)
            
            if not transform_result or not transform_result.get('success', False):
                error_msg = transform_result.get('output', 'Unknown transformation error') if transform_result else 'No result returned'
                
                # Provide more specific error information
                if 'could not compile select expression' in error_msg:
                    st.error(f"❌ XSLT XPath Error: The merged XSLT contains invalid XPath expressions. This usually indicates formatting issues in the merge process.")
                    add_chat_message("assistant", f"❌ XSLT XPath Error: Invalid XPath expressions in merged XSLT. Try regenerating the XSLT or check the merge process.", "text")
                else:
                    st.error(f"❌ Transformation failed: {error_msg}")
                    add_chat_message("assistant", f"❌ Transformation failed: {error_msg}", "text")
                return False
        
        # Store the generated IDoc
        st.session_state.conv["idoc"] = {
            "id": f"idoc_{st.session_state.conv['session_id']}",
            "path": None,
            "content": transform_result.get('output', '')  # Store actual IDoc content
        }
        
        add_chat_message("assistant", "🚀 IDoc generated successfully", "idoc")
        return True
        
    except Exception as e:
        st.error(f"❌ IDoc generation failed: {str(e)}")
        add_chat_message("assistant", f"❌ IDoc generation failed: {str(e)}", "text")
        return False
...
def start_over():
    ...
    # Preserve session ID but reset everything else
    ...
    add_chat_message("assistant", "🔄 Started fresh! Ready to process a new EDI file.", "text")

def render_chat_message(message):
    """Render individual chat messages based on type with error handling"""
    try:
        if message["type"] == "matches":
            ...
            if matches:
                ...
                for i, match in enumerate(matches):
                    try:
                        customer = match['customer_info'].get('customer_name', 'Unknown')
                        score = match.get('structural_score', 0.0)
                        
                        with st.expander(f"📄 #{i+1}: {customer} (Score: {score:.1f}%)", expanded=(i == 0)):
                            col1, col2 = st.columns([1, 3])
                            with col1:
                                st.markdown(f"<div class='match-score'>{score:.1f}%</div>", unsafe_allow_html=True)
                                st.caption("Structural coverage")
                            with col2:
                                st.write(f"**Format:** {match['metadata'].get('format', 'Unknown')}")
                                st.write(f"**Type:** {match['metadata'].get('message_type', 'Unknown')}")
                                st.write(f"**Customer:** {customer}")
                                if match.get('is_cross_format'):
                                    st.success(f"🔄 {match.get('format_pair', '')}")
                            
                            # Selection button
                            if st.button(f"✅ Select #{i+1}", key=f"select_match_{i}"):
                                handle_choose_match(i)
                                st.rerun()
                    except Exception as e:
                        st.error(f"❌ Error rendering match {i+1}: {str(e)}")
            else:
                st.warning("⚠️ No matches available")
        
        elif message["type"] == "analysis":
            # Render coverage analysis in structured table format
            analysis = st.session_state.conv["analysis"]
            if analysis:
                st.markdown("**📊 Coverage Analysis:**")
                try:
                    # Display coverage percentage prominently
                    coverage_pct = analysis.get('coverage_pct', 0)
                    st.markdown(f"<div class='analysis-box'>Coverage: {coverage_pct:.1f}%</div>", unsafe_allow_html=True)
                    
                    # Create tabs for different sections
                    tab1, tab2, tab3 = st.tabs(["✅ Matched Elements", "❌ Missing Elements", "📋 Summary"])
                    
                    with tab1:
                        st.markdown("**Matched Tags:**")
                        matched_tags = analysis.get('matched_tags', [])
                        if matched_tags:
                            cols = st.columns(min(5, len(matched_tags)))
                            for i, tag in enumerate(matched_tags):
                                with cols[i % 5]:
                                    st.success(f"`{tag}`")
                        else:
                            st.info("No matched tags")
                        
                        st.markdown("**Matched Tag Qualifiers:**")
                        matched_tqs = analysis.get('matched_tqs', [])
                        if matched_tqs:
                            for tq in matched_tqs:
                                if len(tq) >= 2:
                                    st.write(f"`{tq[0]}` + `{tq[1]}`")
                        else:
                            st.info("No matched tag qualifiers")
                    
                    with tab2:
                        st.markdown("**Missing Tags:**")
                        missing_tags = analysis.get('missing_tags', [])
                        if missing_tags:
                            cols = st.columns(min(5, len(missing_tags)))
                            for i, tag in enumerate(missing_tags):
                                with cols[i % 5]:
                                    st.error(f"`{tag}`")
                        else:
                            st.success("No missing tags!")
                        ...
                        if missing_tqs:
                            ...
                            st.success("No missing tag qualifiers!")
                    
                    with tab3:
                        st.markdown("**Analysis Summary:**")
                        st.write(f"**Total Coverage:** {coverage_pct:.1f}%")
                        st.write(f"**Matched Tags:** {len(matched_tags)}")
                        st.write(f"**Matched TQs:** {len(matched_tqs)}")
                        st.write(f"**Missing Tags:** {len(missing_tags)}")
                        st.write(f"**Missing TQs:** {len(missing_tqs)}")
        ...
        elif message["type"] == "xslt_template":
            # Render XSLT template viewer
            template = st.session_state.conv["xslt"]["template"]
            if template:
                st.markdown("**✨ Generated XSLT Template:**")
                
                # Show template info
                st.write(f"**Template ID:** {template['id']}")
                st.write(f"**Changes Made:** {len(template.get('changes', []))}")
                
                # Add viewer dropdown
                view_option = st.selectbox(
                    "View XSLT Template:",
                    ["Template Summary", "View Full XSLT", "Changes Made", "Download XSLT"],
                    key=f"xslt_view_{template['id']}"
                )
                
                if view_option == "Template Summary":
                    st.info("XSLT template has been generated successfully!")
                    if template.get('changes'):
                        st.write("**Key Changes:**")
                        for i, change in enumerate(template['changes'][:5]):  # Show first 5 changes
                            st.write(f"{i+1}. {change}")
                        if len(template['changes']) > 5:
                            st.write(f"... and {len(template['changes']) - 5} more changes")
                
                elif view_option == "View Full XSLT":
                    # Show the actual XSLT content
                    xslt_content = template.get('content', '')
                    if xslt_content:
                        st.code(xslt_content, language="xml", line_numbers=True)
                    else:
                        st.warning("No XSLT content available")
                
                elif view_option == "Changes Made":
                    if template.get('changes'):
                        st.write("**All Changes:**")
                        for i, change in enumerate(template['changes']):
                            st.write(f"{i+1}. {change}")
                    else:
                        st.info("No changes were made to the template")
                
                elif view_option == "Download XSLT":
                    xslt_content = template.get('content', '')
                    if xslt_content:
                        st.download_button(
                            "📥 Download Generated XSLT",
                            data=xslt_content,
                            file_name=f"generated_template_{template['id']}.xslt",
                            mime="application/xml"
                        )
                    else:
                        st.warning("No XSLT content available for download")
        
        elif message["type"] == "xslt_final":
            # Render final merged XSLT viewer
            final = st.session_state.conv["xslt"]["final"]
            if final:
                st.markdown("**🔄 Final Merged XSLT:**")
                
                st.write(f"**Final ID:** {final['id']}")
                st.write(f"**Summary:** {final.get('diff_summary', 'Auto-merge completed')}")
                
                # Add viewer dropdown
                view_option = st.selectbox(
                    "View Final XSLT:",
                    ["Merge Summary", "View Merged XSLT", "Download XSLT"],
                    key=f"final_view_{final['id']}"
                )
                
                if view_option == "Merge Summary":
                    st.success("XSLT templates have been successfully merged!")
                    st.write("The merged template is ready for transformation.")
                
                elif view_option == "View Merged XSLT":
                    merged_content = final.get('content', '')
                    if merged_content:
                        st.code(merged_content, language="xml", line_numbers=True)
                        
                        # Add debug information if there were validation warnings
                        if hasattr(st.session_state, 'xslt_validation_warning'):
                            st.warning("⚠️ This XSLT had validation warnings during merge. Check for XPath syntax issues.")
                    else:
                        st.warning("No merged XSLT content available")
                
                elif view_option == "Download XSLT":
                    merged_content = final.get('content', '')
                    if merged_content:
                        st.download_button(
                            "📥 Download Merged XSLT",
                            data=merged_content,
                            file_name=f"merged_template_{final['id']}.xslt",
                            mime="application/xml"
                        )
                    else:
                        st.warning("No merged XSLT content available for download")
        
        elif message["type"] == "idoc":
            # Render IDoc viewer
            idoc = st.session_state.conv["idoc"]
            if idoc:
                st.markdown("**🚀 Generated IDoc:**")
                
                st.write(f"**IDoc ID:** {idoc['id']}")
                
                # Add viewer dropdown
                view_option = st.selectbox(
                    "View IDoc:",
                    ["IDoc Summary", "View IDoc Content", "Download IDoc"],
                    key=f"idoc_view_{idoc['id']}"
                )
                
                if view_option == "IDoc Summary":
                    st.success("IDoc has been successfully generated!")
                    st.write("Your EDI file has been transformed into SAP IDoc format.")
                
                elif view_option == "View IDoc Content":
                    idoc_content = idoc.get('content', '')
                    if idoc_content:
                        st.code(idoc_content, language="xml", line_numbers=True)
                    else:
                        st.warning("No IDoc content available")
                
                elif view_option == "Download IDoc":
                    idoc_content = idoc.get('content', '')
                    if idoc_content:
                        st.download_button(
                            "📥 Download IDoc",
                            data=idoc_content,
                            file_name=f"idoc_{idoc['id']}.xml",
                            mime="application/xml"
                        )
                    else:
                        st.warning("No IDoc content available for download")
        
        elif message["type"] == "download":
            # Render download button
            st.markdown("**📥 Download Ready:**")
            st.success("Your IDoc is ready for download!")
            if st.button("📥 Download IDoc", key="download_idoc"):
            ...
            st.markdown(f"{role_icon} **{message['role'].title()}** ({timestamp}): {message['content']}")
    
    except Exception as e:
        st.error(f"❌ Error rendering message: {str(e)}")
        st.text(f"Raw message: {str(message)}")

def render_action_buttons(options, current_step):
    """Render action buttons based on current step with error handling"""
    try:
        if not options:
            return
        
        cols = st.columns(len(options))
        timestamp = str(st.session_state.conv["session_id"])[-8:]  # Use last 8 chars of session ID
        
        for i, option in enumerate(options):
            with cols[i]:
                if st.button(option, key=f"action_{option}_{timestamp}"):
                    handle_action(option, current_step)
                    st.rerun()
    
    except Exception as e:
        st.error(f"❌ Error rendering action buttons: {str(e)}")

def handle_action(action, current_step):
    """Handle action button clicks with comprehensive error handling"""
    try:
        if action == "Upload EDI File":
            # File uploader will be handled in the UI
            pass
        elif action == "🔍 Find Matches":
            if not st.session_state.conv["upload"]:
                st.error("❌ Please upload an EDI file first")
                add_chat_message("assistant", "❌ Please upload an EDI file first", "text")
                return
            if not st.session_state.llm_engine:
                st.error("❌ LLM engine not configured. Please check your Azure OpenAI settings.")
                add_chat_message("assistant", "❌ LLM engine not configured. Please check your Azure OpenAI settings.", "text")
                return
            handle_show_matches()
        elif action == "Select Match":
            # Match selection is handled in render_chat_message
            pass
        elif action == "🔬 Analyze Coverage":
            if not st.session_state.conv["chosen_match"]:
                st.error("❌ Please select a match first")
                add_chat_message("assistant", "❌ Please select a match first", "text")
                return
            if not st.session_state.llm_engine:
                st.error("❌ LLM engine not configured. Please check your Azure OpenAI settings.")
                add_chat_message("assistant", "❌ LLM engine not configured. Please check your Azure OpenAI settings.", "text")
            ...
            if not st.session_state.llm_engine:
                ...
                add_chat_message("assistant", "❌ LLM engine not configured. Please check your Azure OpenAI settings.", "text")
                return
            handle_generate_xslt()
        elif action == "🔄 Auto-Merge":
            if not st.session_state.conv["xslt"]["template"]:
                st.error("❌ Please generate XSLT template first")
                add_chat_message("assistant", "❌ Please generate XSLT template first", "text")
                return
            handle_automerge()
        elif action == "🚀 Build IDoc":
            if not st.session_state.conv["xslt"]["final"]:
                st.error("❌ Please complete auto-merge first")
                add_chat_message("assistant", "❌ Please complete auto-merge first", "text")
                return
            handle_build_idoc()
        elif action == "📥 Download IDoc":
            if not st.session_state.conv["idoc"]:
                st.error("❌ IDoc not generated yet")
                add_chat_message("assistant", "❌ IDoc not generated yet", "text")
                return
            st.info("Download functionality would be implemented here")
            ...
            add_chat_message("assistant", f"❌ Unknown action: {action}", "text")
    except Exception as e:
        st.error(f"❌ Action failed: {str(e)}")
        add_chat_message("assistant", f"❌ Action failed: {str(e)}", "text")

def render_context_bar():
    """Render compact context information with error handling"""
    try:
        ...
        if conv["upload"]:
            context_items.append(f"📄 {conv['upload']['filename']}")
        
        if conv["chosen_match"]:
            customer_name = conv["chosen_match"]["customer_info"].get("customer_name", "Unknown")
            context_items.append(f"🏢 {customer_name}")
        
        if conv["analysis"]:
            # Extract coverage percentage if available
            coverage = conv["analysis"].get("coverage_percentage", 0)
            context_items.append(f"📊 {coverage}% coverage")
        
        # Current step
        current_step_info = get_next_step(conv)
        context_items.append(f"📍 {current_step_info['step']}")
        
        if context_items:
            st.markdown(" | ".join(context_items))
    
    except Exception as e:
        st.error(f"❌ Error rendering context bar: {str(e)}")

def render_details_panel():
    """Render collapsible details panel with error handling"""
    try:
        conv = st.session_state.conv
        ...
        if conv["analysis"]:
            with st.expander("📊 Analysis Details", expanded=False):
                try:
                    st.json(conv["analysis"])
                except Exception as e:
                    st.error(f"❌ Error rendering analysis details: {str(e)}")
                    st.text(str(conv["analysis"]))
        
        if conv["xslt"]["template"]:
            with st.expander("✨ XSLT Template Details", expanded=False):
                try:
                    template = conv["xslt"]["template"]
                    st.write(f"**ID:** {template['id']}")
                    st.write(f"**Changes:** {len(template.get('changes', []))}")
                    if template.get('changes'):
                        st.json(template['changes'])
                except Exception as e:
                    st.error(f"❌ Error rendering XSLT template details: {str(e)}")
        
        if conv["xslt"]["final"]:
            with st.expander("🔄 Final XSLT Details", expanded=False):
                try:
                    final = conv["xslt"]["final"]
                    st.write(f"**ID:** {final['id']}")
                    st.write(f"**Summary:** {final.get('diff_summary', 'N/A')}")
                except Exception as e:
                    st.error(f"❌ Error rendering final XSLT details: {str(e)}")
    
    except Exception as e:
        st.error(f"❌ Error rendering details panel: {str(e)}")
...
def main():
    ...
    with tab2:
        # Conversational EDI Workflow Interface
        
        if not st.session_state.database_ready:
            st.warning("⚠️ Please build the database first (go to 'Build Database' tab)")
            st.stop()
        
        # Initialize chat if empty
        if not st.session_state.chat_messages:
            add_chat_message("assistant", "👋 Welcome to the Conversational EDI Workflow! I'll guide you through finding the best template and generating your IDoc step by step.", "text")
        
        # Get current step
        current_step_info = get_next_step(st.session_state.conv)
        current_step = current_step_info["step"]
        
        # Chat Display Area (top section)
        st.markdown("### 💬 EDI Workflow Assistant")
        ...
        # Current Action Area (middle section)
        st.markdown("---")
        st.markdown(f"**{current_step_info['prompt']}**")
        
        # Handle file upload for intake step
        if current_step == "intake":
            uploaded_file = st.file_uploader("Upload EDI File", type=['edi','txt','x12'], key="edi_uploader")
            if uploaded_file:
                if handle_upload_edi(uploaded_file):
                    st.rerun()
        
        # Action buttons
        render_action_buttons(current_step_info['options'], current_step)
        
        # Context Bar (below chat)
        st.markdown("---")
        render_context_bar()
